//
//  FTFTagCollectionViewFlowLayout.m
//  FTFTalk
//
...
@interface FTFTagCollectionViewFlowLayout ()
...
@implementation FTFTagCollectionViewFlowLayout
...
- (void)prepareLayout {
  ...
  for (NSInteger i = 0; i < itemCount; i++) {
    ...
    if ((originX + itemSize.width + self.sectionInset.right / 2) >
    ...
    attributes.frame =
        CGRectMake(originX, originY, itemSize.width, itemSize.height);
...
- (CGSize)itemSizeForIndexPath:(NSIndexPath *)indexPath {
  if ([self.delegate respondsToSelector:@selector(collectionView:
    ...
    self.itemSize = [self.delegate collectionView:self.collectionView
                                           layout:self
